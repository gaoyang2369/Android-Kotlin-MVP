...
Android Kotlin MVP：只监听 `com.tencent.mm` 的无障碍窗口内容，命中写死规则后用 `WindowManager.TYPE_APPLICATION_OVERLAY` 显示 `assets/sticker.png`，离开目标页面后隐藏。
...
`app/src/main/assets/sticker.png` 已从 `pic.jpg` 中裁出，内容是带浅色背景的“宋高杨”姓名区域。
...
6. 点击“测试显示贴图”，确认贴图能显示且不能被触摸拖动；点击“测试隐藏贴图”确认能隐藏。
7. 当前识别规则是页面可访问文本中包含“确认入场”。如果微信无障碍读不到按钮文字，再把 `TargetPageConfig.requiredTexts` 改成页面上其他稳定可读文字。
